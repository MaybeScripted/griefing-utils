...
public class NotificationToast implements Toast {
    ...
    public Visibility draw(DrawContext ctx, ToastManager manager, long startTime) {
        ...
        if (startTime < 1500L) {
            ctx.drawText(textRenderer, title, 30, height, 0xffffffff, true);
        } else {
            if(!Objects.equals(line2, Text.empty())) {
                ctx.drawText(textRenderer, line1, 30, height - (int) (textRenderer.fontHeight * .5f) - 1, 0xFFFFFFFF, true);
                ctx.drawText(textRenderer, line2, 30, height + (int) (textRenderer.fontHeight * .5f) + 1, 0xFFFFFFFF, true);
            } else {
                ctx.drawText(textRenderer, line1, 30, height, 0xFFFFFFFF, true);
            }
        }
